CREATE TABLE `__new_devin_sessions` (
	...
	CONSTRAINT "devin_sessions_outcome_check" CHECK(("status" IN ('succeeded', 'failed') AND "outcome" IS NOT NULL AND "outcome" IN ('fixed', 'needs_human', 'not_reproducible', 'failed')) OR ("status" NOT IN ('succeeded', 'failed') AND "outcome" IS NULL))
);
--> statement-breakpoint
INSERT INTO `__new_devin_sessions`(`id`, `github_delivery_id`, `status`, `outcome`, `devin_session_id`, `pr_number`, `attempts`, `claim_version`, `recovery_empty_checks`, `recovery_blocked`, `inserted_at`, `updated_at`) SELECT `id`, `github_delivery_id`, `status`, CASE WHEN `status` = 'succeeded' THEN 'needs_human' WHEN `status` = 'failed' THEN 'failed' ELSE NULL END, `devin_session_id`, `pr_number`, `attempts`, `claim_version`, `recovery_empty_checks`, `recovery_blocked`, `inserted_at`, `updated_at` FROM `devin_sessions`;--> statement-breakpoint
